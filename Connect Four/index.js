document.addEventListener('DOMContentLoaded',()=>
{
        ...
        for(let i=0;i<winningArrays.length;i++){
            ...
            //check those squares to see if they all have the class of player-one
             if(square1.classList.contains('player-one') && square2.classList.contains('player-one')
              && square2.classList.contains('player-one') && square4.classList.contains('player-one')){
                result.innerHTML="Player-1 Wins"
              }
           //check those squares to see if they all have the class of player-one
              if(square1.classList.contains('player-two') && square2.classList.contains('player-two')
              && square2.classList.contains('player-two') && square4.classList.contains('player-two')){
                  result.innerHTML="Player-1 Wins"
              }
              }
        }
    ...
    for(let i=0;i<squares.length;i++){
        squares[i].onclick=()=>{
            //check if the square below your current square is taken, so you can play on top of it
